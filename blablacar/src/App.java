...
public class App {
    ...
    private static void historicoCaronas() {
        ...
        System.out.println("\t└─────────────────────────────────────┘");
        scanner.nextLine();
        List<Viagem> viagensPassageiro = viagemRepo.listarViagensPorPassageiro(usuarioLogado);
        if (viagensPassageiro.isEmpty()) {
            System.out.println("\n Você não possui caronas no histórico.");
            pausar();
            return;
        }
        System.out.println("\n Suas caronas ativas:");
        for (Viagem v : viagensPassageiro) {
            if (!v.getStatus().equals("CONCLUIDO")) {
                System.out.println("─────────────────────────────────────────────────────");
                System.out.println(" ID: " + v.getId());
                System.out.println(" De: " + v.getCidadeOrigem() + " → Para: " + v.getCidadeDestino());
                System.out.println(" Motorista: " + v.getMotorista().getNome());
                System.out.println(" Veículo: " + v.getVeiculo().getMarca() + " "
                        + v.getVeiculo().getModelo() + " (" + v.getVeiculo().getAno() + ")");
                System.out.println(" Avaliação da carona: " + String.format("%.1f", v.getAvaliacao()));
                System.out.println(" Data: " + v.getData());
                System.out.println(" Status: " + v.getStatus());
            }
        System.out.println("─────────────────────────────────────────────────────");
        System.out.println("\n Suas caronas concluídas:");
        listarViagensMotoristaComStatus("CONCLUIDA");
        }
        System.out.println("─────────────────────────────────────────────────────");
    }
    ...
    private static void ofertarCarona() {
        ...
        scanner.nextLine();
        if(usuarioLogado.getMotorista().getVeiculos() == null || usuarioLogado.getMotorista().getVeiculos().isEmpty()){
            System.out.println("\n  Veículo não cadastrado. Carona não criada.");
            pausar();
            return;
        }
    ...
    private static void buscarCaronas() {
        ...
        if (verificarIdViagem(idViagem)) {
            if (verificarPassageiro()) {

            viagemAtual = viagemRepo.reservarViagemPorId(idViagem, pessoas, usuarioLogado);
            reservarCarona(viagemAtual);
            }
        }else {
            System.out.println("\n  Reserva cancelada ou viagem não encontrada.");
            pausar();
        }
    }
    private static boolean verificarIdViagem(int idViagem) {
        if (idViagem > 0 && viagemRepo.getViagemPorId(idViagem) != null) {
            return true;
        }
        return false;
    }

    private static void reservarCarona(Viagem viagemAtual) {
        System.out.println("\n\t┌─────────────────────────────────────┐");
    ...
    private static void minhasCaronasOfertadas() {
        ...
        System.out.println("─────────────────────────────────────────────────────");
        System.out.println("Caronas pendentes:\n");
        listarViagensMotoristaComStatus("PENDENTE");

        System.out.println("─────────────────────────────────────────────────────");
        System.out.println("Caronas cheias:\n");
        listarViagensMotoristaComStatus("CHEIA");
        ...
        switch (opcao) {
            case 2 -> {
                ...
                int ocupados = viagemSelecionada.getTotalPassageiros();
                if (lugares > ocupados) {
    ...
    private static void minhasReservas() {
        ...
        switch (opcao) {
            case 1 -> {
                System.out.print("Quantos lugares voce deseja realmente reservar? ");
                int novasPessoas = scanner.nextInt();
                try {
                    viagemRepo.atualizarReserva(usuarioLogado, novasPessoas, viagem);
                    System.out.println("\n Reserva atualizada com sucesso!");
                } catch (Exception e) {
                    System.out.println("\n Falha ao atualizar reserva: " + e.getMessage());
                }
            }
            case 2 -> {
                boolean cancelada = viagemRepo.cancelarReserva(viagem, usuarioLogado);
                if (cancelada) {
    ...
    public static void listarViagensMotoristaComStatus(String status) {
            for (Viagem v : viagemRepo.listarViagensPorMotorista(usuarioLogado)) {
                if(v.getStatus().equals(status)){
                System.out.println("\n ID: " + v.getId());
                System.out.println(" De " + v.getCidadeOrigem() + " para " + v.getCidadeDestino());
                System.out.println(" Passageiros: "+ v.getTotalPassageiros() + " de " + v.getVagas());
                System.out.println(" Data: " + v.getData());
                System.out.println(" Status: " + v.getStatus());
                System.out.println("─────────────────────────────────────────────────────");
            }
